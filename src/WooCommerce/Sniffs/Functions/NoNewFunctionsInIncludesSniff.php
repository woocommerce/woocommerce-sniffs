 ...
 *
 * Detects NEW function declarations in the includes directory and reports them as errors.
 * Only flags functions that are new (not in git history).
 */
class NoNewFunctionsInIncludesSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->isMethodInsideClass($phpcsFile, $stackPtr)) {
            return;
        }

        // Check if this function is new (not in git history)
        if (!$this->isNewFunction($phpcsFile, $stackPtr)) {
            return;
        }

        // This is a new standalone function in the includes directory - report as error
        $phpcsFile->addError(
    ...
    /**
     * Check if the function is a method inside a class
     *
     * @param File $phpcsFile The file being scanned.
     * @param int  $stackPtr  The position of the current token in the stack passed in $tokens.
     *
     * @return bool
     */
    private function isMethodInsideClass(File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if ($prevToken !== false && $tokens[$prevToken]['code'] === T_CLOSE_CURLY_BRACKET) {
            ...
            if ($classToken !== false) {
                return true; // This is a method, not a standalone function
            }
        }
        ...
        if ($classToken !== false && $tokens[$classToken]['line'] < $functionLine) {
            ...
            if ($classEnd > $stackPtr) {
                return true; // This is a method inside a class
            }
        }
    ...
    /**
     * Check if the function is new (not in git history)
     *
     * @param File $phpcsFile The file being scanned.
     * @param int  $stackPtr  The position of the current token in the stack passed in $tokens.
     *
     * @return bool
     */
    private function isNewFunction(File $phpcsFile, $stackPtr)
    {
        $fileName = $phpcsFile->getFilename();
        $tokens = $phpcsFile->getTokens();
        ...
        $functionName = $tokens[$functionName]['content'];
        $functionLine = $tokens[$stackPtr]['line'];
        
        // Check if this function exists in git history
        $command = sprintf(
            'git log -L:%d,%d:%s --oneline 2>/dev/null | head -1',
            $functionLine,
            $functionLine,
        ...
        // If git log returns empty, the function is new
